...
public class Car {
    ...
    //This method is used by the manager to add vehicles to the lot...
    public static void addVehiclesToLot (ArrayList<Car> carList) {
        int newCarID = 4;
        Scanner scnr = new Scanner(System.in);
        Car car = new Car();
        boolean managerDone = false;
        do {
            System.out.println("Enter new car to inventory?");
            String yesOrNo = scnr.next();
            if (yesOrNo.equalsIgnoreCase("y")) {
                try {
                    System.out.println("\nCar ID (An Integer): ");
                    car.setCarID(newCarID++);
                    System.out.println("Mileage (An Integer): ");
                    car.setCarMileage(scnr.nextInt());
                    System.out.println("Description (A String): ");
                    car.setCarDescription(scnr.nextLine());
                    scnr.reset();
                    System.out.println("Color (A String): ");
                    car.setCarColor(scnr.next());
                    System.out.println("Date Added (A String in the form MM/DD/YYYY): ");
                    car.setDateAdded(scnr.next());
                    System.out.println("Model Year (An Integer): ");
                    car.setModelYear(scnr.nextInt());
                    System.out.print("Availability: ");
                    System.out.println("(1)Available\n(2)Unavailable\nEnter the corresponding number: ");
                    boolean availabilityEntered = false;
                    do {
                        String managerInput = scnr.next();
                        if (managerInput.equals('1')) {
                            car.setAvailable(true);
                            System.out.println("Set to: Available");
                            availabilityEntered = true;
                        } else if (managerInput.equals('2')) {
                            car.setAvailable(false);
                            System.out.println("Set to: Unavailable");
                            availabilityEntered = true;
                        } else {
                            System.out.println("Invalid availability entry. Please Try again.");
                        }
                    } while (!availabilityEntered);
                    System.out.printf("Car Price (A Double with 2 decimal places): ");
                    car.setCarPrice(scnr.nextDouble());
                    System.out.println();
                } catch (InputMismatchException e) {
                    System.out.println("ERROR: InputMismatchException. Please re-enter vehicle data.");
                }
            }
