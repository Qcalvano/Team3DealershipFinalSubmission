...
public class User {
     ...
     * @param custList  An ArrayList of Customer objects.
     * @author          Quenten Calvano
     * @version         1.0
     * @since           2020-24-06
     */
    public static void userMenu(ArrayList<Car> carList, ArrayList<Dealership> dList, ArrayList<User> uList, ArrayList<SalesAssociate> saList, ArrayList<Customer> custList) {
        Scanner scnr = new Scanner(System.in);
                        ...
                        for (User user : uList) {
                            if (password.equals(user.getPassword())) {
                                if (user.getTypeID() == 0){
                                    ...
                                    do {
                                        System.out.println("(1)Enter a new customer contact entry." +
                                                "\n(2)View your customer contact entry.\nType 'e' to exit.");
                                        String custInput = scnr.next();
                                        if (custInput.equals('1')) {
                                            Customer.addCustomerData(custList);
                                        }
                                        else if (custInput.equals('2')){
                                            Customer.printCustomer(custList);
                                        }
                                        else if (custInput.equals('e')) {
                                            custDone = true;
                                        }
                                        else {
                                            System.out.println("ERROR: Invalid user choice please try again.");
                                        }
                                    } while (!custDone);
                                    ...
                                    System.out.println("Would you like to enter(1) or view(2) sales data? Type 'e' to exit.");
                                    boolean exitSAMenu = false;
                                    do{
                                        String userChoice = scnr.next();
                                        if (userChoice.equals("1")){
                                            SalesAssociate.addSalesForAssociateUser(saList, carList);
                                        }
                                        else if (userChoice.equals("2")){
                                            System.out.println("Enter Sales ID: ");
                                            String salesID = scnr.next();

                                            for (SalesAssociate sales : saList) {
                                                if (sales.getSalesmanId() == Integer.parseInt(salesID)) {
                                                    sales.viewRecordSales();
                                                    System.out.println();
                                                    sales.viewCustomerContacts();
                                                }
                                                else{
                                                    System.out.println("ERROR: Incorrect Sales ID");
                                                }
                                            }
